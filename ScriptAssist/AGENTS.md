...
- `ILanguage.Browse` groups native plugins, imported aliases, This file, and (VS) installed packages not already in `ScriptModules`. Column-0 `class Name` in an imported or listed package is an insertable export; nested methods stay out. Constructor insight reads that class body's `def __init__` at the class indent, dropping a leading `self`/`cls`. A `@dataclass` with no `__init__` uses that class body's fields. Enum/Flag/TypedDict/NamedTuple/ABC/Error bases are names, not calls: insert `module.Name` and hint the class-body constants. No such header or type-name base keeps parameters unknown. Do not walk bases. Skip `vs` / `vapoursynth` script modules. AviSynth `User` is labeled Autoload. Function names are sorted ordinal-ignore-case. VS groups are This file then ordinal; AVS buckets stay This file / Internal / Plugin / Autoload. Unimported VS packages set `BrowseFunction.Import` and insert `module.Name()`; the host adds a column-0 `import module` when missing. `from module import` does not bind the module name.
